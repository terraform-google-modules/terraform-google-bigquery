...
| materialized\_views | A list of objects which includes view\_id, and view query | <pre>list(object({<br>    view_id             = string,<br>    query               = string,<br>    enable_refresh      = bool,<br>    refresh_interval_ms = string,<br>    labels              = map(string),<br>  }))</pre> | `[]` | no |

...
| routines | A list of objects which include table\_id, schema, clustering, time\_partitioning, range\_partitioning, expiration\_time and labels. | <pre>list(object({<br>    routine_id          = string,<br>    routine_type        = string,<br>    routine_language    = string,<br>    definition_body     = string,<br>    return_type         = string,<br>    routine_description = string,<br>    arguments = object({<br>      name            = string,<br>      data_type       = string,<br>      argument_kind   = string,<br>      mode            = string, <br>    }),<br>  }))</pre> | `[]` | no |
...
| routine\_ids | Friendly names for any external tables being provisioned |

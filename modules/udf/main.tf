...
locals {
  ...
  function_name_regex = "CREATE\\s+(?:OR REPLACE\\s+)?FUNCTION\\s+(?:IF NOT EXISTS)?\\s*([a-zA-Z0-9_]+)(?=\\s*\\()"
  function_map = {
...
module "udf" {
  ...
  create_cmd_body = "--project_id=${var.project_id} --dataset_id=${var.dataset_id} query --nouse_legacy_sql '${each.value}'"

  destroy_cmd_body = "--project_id=${var.project_id} --dataset_id=${var.dataset_id} query --nouse_legacy_sql 'DROP FUNCTION IF EXISTS ${each.key}'"
}

...
CREATE TABLE ${project_id}.${google_bigquery_dataset.ds_edw.dataset_id}.inventory_items (id VARCHAR, product_id VARCHAR, created_at TIMESTAMP, sold_at TIMESTAMP, cost NUMERIC, product_category VARCHAR, product_name VARCHAR, product_brand VARCHAR, product_retail_price NUMERIC, product_department VARCHAR, product_sku VARCHAR, product_distribution_center_id VARCHAR);
CREATE TABLE ${project_id}.${google_bigquery_dataset.ds_edw.dataset_id}.order_items (id INTEGER, order_id INTEGER, user_id INTEGER, product_id INTEGER, inventory_item_id INTEGER, status VARCHAR, created_at TIMESTAMP, shipped_at TIMESTAMP, delivered_at TIMESTAMP, returned_at TIMESTAMP, sale_price NUMERIC);
...
FROM ${project_id}.${google_bigquery_dataset.ds_edw.dataset_id}.inventory_items AS inventory
  INNER JOIN ${project_id}.${google_bigquery_dataset.ds_edw.dataset_id}.order_items AS order_items

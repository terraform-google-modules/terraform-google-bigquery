...
CREATE OR REPLACE MATERIALIZED VIEW `${project_id}.ds_edw.lookerstudio_report_distribution_centers`
  CLUSTER BY distribution_center_name, year, week_name, week_number
AS
WITH OrdersData AS
(
  SELECT  dc.name                                               AS distribution_center_name,
          ...
          COUNT(product_id)                                     AS products_ordered_count,
  ...
  JOIN
    `${project_id}.ds_edw.inventory_items` AS inventory_items USING (product_id) AND order_items.inventory_item_id = inventory_items.id
  JOIN
...
(
  SELECT  distribution_center_name,
          ...
          LAG(products_ordered_count)          OVER (PARTITION BY distribution_center_name ORDER BY year DESC, week_number DESC) AS prior_products_ordered_count,
          LAG(orders_count)                    OVER (PARTITION BY distribution_center_name ORDER BY year DESC, week_number DESC) AS prior_orders_count,
          LAG(profit_total)                    OVER (PARTITION BY distribution_center_name ORDER BY year DESC, week_number DESC) AS prior_profit_total,
          LAG(profit_avg)                      OVER (PARTITION BY distribution_center_name ORDER BY year DESC, week_number DESC) AS prior_profit_avg,
          LAG(inventory_sold_cost_total)       OVER (PARTITION BY distribution_center_name ORDER BY year DESC, week_number DESC) AS prior_inventory_sold_cost_total,
          LAG(inventory_sold_cost_avg)         OVER (PARTITION BY distribution_center_name ORDER BY year DESC, week_number DESC) AS prior_inventory_sold_cost_avg,
          LAG(shipping_hours)                  OVER (PARTITION BY distribution_center_name ORDER BY year DESC, week_number DESC) AS prior_shipping_hours,
          LAG(processing_hours)                OVER (PARTITION BY distribution_center_name ORDER BY year DESC, week_number DESC) AS prior_processing_hours,
          LAG(order_to_delivery_hours)         OVER (PARTITION BY distribution_center_name ORDER BY year DESC, week_number DESC) AS prior_order_to_delivery_hours
    FROM OrdersData
)
, PercentChange AS
(
  SELECT  distribution_center_name,
          ...
          processing_hours,
          order_to_delivery_hours,
          prior_products_ordered_count,
          prior_orders_count,
          prior_profit_total,
          prior_profit_avg,
          prior_inventory_sold_cost_total,
          prior_inventory_sold_cost_avg,
          prior_shipping_hours,
          prior_processing_hours,
          prior_order_to_delivery_hours,
          SAFE_DIVIDE(CAST(products_ordered_count - prior_products_ordered_count AS NUMERIC) , CAST(prior_products_ordered_count AS NUMERIC)) AS percent_change_products_ordered_count,
          ...
          SAFE_DIVIDE((inventory_sold_cost_total - prior_inventory_sold_cost_total) , prior_inventory_sold_cost_total) AS percent_change_inventory_sold_cost_total,
          SAFE_DIVIDE((inventory_sold_cost_avg - prior_inventory_sold_cost_avg) , prior_inventory_sold_cost_avg) AS percent_change_inventory_sold_cost_avg,
          SAFE_DIVIDE((shipping_hours - prior_shipping_hours) , prior_shipping_hours) AS percent_change_shipping_hours,
          SAFE_DIVIDE((processing_hours - prior_processing_hours) , Prior_Week_Total_Tip_Amount) AS percent_change_processing_hours,
          SAFE_DIVIDE((order_to_delivery_hours - prior_order_to_delivery_hours) , prior_order_to_delivery_hours) AS percent_change_order_to_delivery_hours
  FROM LagPercents
)
SELECT *
FROM PercentChange
...
CREATE OR REPLACE MATERIALIZED VIEW `${project_id}.ds_edw.lookerstudio_report_profit`
  CLUSTER BY day, product_dept_cat
AS
with SubsetInventory AS(
  SELECT
    SUM(ROUND(product_retail_price,2)) AS product_retail_price,
...
Inventory7d AS (
  SELECT
    product_dept_cat,
    sold_at_day AS day,
    SUM(ROUND(product_retail_price,2)) OVER (PARTITION BY product_dept_cat ORDER BY UNIX_DATE(sold_at_day) ASC RANGE BETWEEN 6 PRECEDING and CURRENT ROW) AS revenue_last_7d,
    SUM(ROUND(cost,2)) OVER (PARTITION BY product_dept_cat ORDER BY UNIX_DATE(sold_at_day) ASC RANGE BETWEEN 6 PRECEDING and CURRENT ROW) AS cost_last_7d,
  FROM
    SubsetInventory
),

...
CREATE OR REPLACE VIEW `${project_id}.${google_bigquery_dataset.ds_edw.dataset_id}.lookerstudio_report_distribution_centers`
OPTIONS(
...
(
  ...
  JOIN
    `${project_id}.${google_bigquery_dataset.ds_edw.dataset_id}.inventory_items` AS inventory_items ON order_items.product_id = inventory_items.product_id AND order_items.inventory_item_id = inventory_items.id
  JOIN
    `${project_id}.${google_bigquery_dataset.ds_edw.dataset_id}.distribution_centers` AS dc ON inventory_items.product_distribution_center_id = dc.id
  WHERE
...
CREATE OR REPLACE VIEW `${project_id}.${google_bigquery_dataset.ds_edw.dataset_id}.lookerstudio_report_profit`
OPTIONS(
    labels=[("data-warehouse","true")]
)
AS
